...
int mxt_t15_keys[] = { KEY_MENU, KEY_HOME, KEY_BACK, KEY_POWER};
struct mxt_info {
	u8 family_id;
	u8 variant_id;
	u8 version;
...
static ssize_t mxt_plugin_stylus_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count);
static ssize_t mxt_plugin_wakeup_dclick_show(struct device *dev,
	struct device_attribute *attr, char *buf);
static ssize_t mxt_plugin_wakeup_dclick_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count);
...
#if defined(CONFIG_MXT_PLUGIN_SUPPORT)
	idx = mxt_plugin_hook_t24(&data->plug, msg);
#endif
	if (idx >= 0) {
		if (idx >= 0 && idx < data->pdata->t15_num_keys) {
			input_event(input_dev, EV_KEY,
					data->pdata->t15_keymap[idx], 1);
			input_sync(input_dev);
			input_event(input_dev, EV_KEY,
					data->pdata->t15_keymap[idx], 0);
			input_sync(input_dev);

			set_bit(MXT_WK_DETECTED, &data->enable_wakeup);
		} else {
			dev_info(dev, "T24 discard unused key %d\n",
				idx);
		}
	}
}
...
static ssize_t mxt_t25_selftest_store_enable(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	struct mxt_data *data = dev_get_drvdata(dev);
	u32 cmd;
...
#if defined(CONFIG_MXT_PLUGIN_SUPPORT)
static int mxt_proc_t61_messages(struct mxt_data *data, u8 *msg)
{
	struct input_dev *input_dev = data->input_dev;
	struct device *dev = &data->client->dev;
	int id;
	...
	idx = mxt_plugin_hook_t61(&data->plug, id, status);

	if (idx >= 0) {
		if (idx >= data->pdata->t15_num_keys)
			idx  = data->pdata->t15_num_keys - 1;
		input_event(input_dev, EV_KEY,
				data->pdata->t15_keymap[idx], 1);
		input_sync(input_dev);
		input_event(input_dev, EV_KEY,
		...
		dev_info(dev, "T92 discard unused key idx %d\n",
			idx);
	}
...
static void mxt_proc_T81_messages(struct mxt_data *data, u8 *msg)
{
...
#if defined(CONFIG_MXT_PLUGIN_SUPPORT)
	idx = mxt_plugin_hook_t81(&data->plug, msg);
#endif
	...
	if (idx >= 0) {
		...
		dev_info(dev, "T81 discard unused key idx %d\n",
			idx);
	}
}
...
static void mxt_proc_T92_messages(struct mxt_data *data, u8 *msg)
{
	...
	dev_info(dev, "T92 Status 0x%x Info: %x %x %x %x %x %x\n",
		status,
		msg[2],
		msg[3],
		msg[4],
...
#if defined(CONFIG_MXT_PLUGIN_SUPPORT)
	idx = mxt_plugin_hook_t92(&data->plug, msg);
#endif

	if (status & 0x80)
		dev_info(dev, "T92 stroke %d\n",idx);
	else
		dev_info(dev, "T92 symbol %d\n",idx);
...
#if defined(CONFIG_MXT_PLUGIN_SUPPORT)
	idx = mxt_plugin_hook_t93(&data->plug, msg);
#endif
	if (idx >= 0) {
		if (idx >= data->pdata->t15_num_keys)
			idx  = data->pdata->t15_num_keys - 1;
		...
		dev_info(dev, "T93 discard unused key %d\n",
			idx);
	}
}
...
static int mxt_check_reg_init(struct mxt_data *data)
{
	...
	if (info_crc == data->info_crc) {
		...
		if (mxt_plugin_cal_t37_check_and_calibrate(&data->plug, true) != 0)
#endif
...
#if defined(CONFIG_MXT_REPORT_VIRTUAL_KEY_SLOT_NUM)
	if (data->pdata->max_y_t < data->max_y)
		data->pdata->max_y_t = data->max_y;
#endif
	dev_info(&client->dev,
...
static int mxt_initialize_t100_input_device(struct mxt_data *data)
{
...
#endif
	{
		...
		for (i = 0; i < data->pdata->t15_num_keys; i++) {
			set_bit(data->pdata->t15_keymap[i], input_dev->keybit);
...
static DEVICE_ATTR(t19, S_IWUSR | S_IRUSR /*| S_IWGRP | S_IWOTH */, mxt_t19_gpio_show,
			mxt_t19_gpio_store);
static DEVICE_ATTR(t25, S_IWUSR | S_IRUSR | S_IRUGO/*| S_IWGRP | S_IWOTH*/, mxt_t25_selftest_show,
			mxt_t25_selftest_store);
static DEVICE_ATTR(t25_enable, S_IWUSR | S_IRUSR | S_IRUGO, mxt_t25_selftest_store_enable,
		   NULL);
...
static DEVICE_ATTR(en_wk_dclick, S_IWUSR | S_IRUSR, mxt_plugin_wakeup_dclick_show,
			mxt_plugin_wakeup_dclick_store);
static DEVICE_ATTR(en_wk_gesture, S_IWUSR | S_IRUSR, mxt_plugin_wakeup_gesture_show,
...
static struct attribute *mxt_attrs[] = {
	...
	&dev_attr_en_wk_dclick.attr,
	&dev_attr_en_wk_gesture.attr,
#endif
#if defined(CONFIG_MXT_SELFCAP_TUNE)
...
#if defined(CONFIG_MXT_PLUGIN_SUPPORT)
		/* Recalibrate since chip has been in deep sleep */
		if (mxt_plugin_cal_t37_check_and_calibrate(&data->plug, false) != 0)
#endif
			mxt_t6_command(data, MXT_COMMAND_CALIBRATE, 1, false);
#if defined(CONFIG_MXT_PLUGIN_SUPPORT)
...
#endif
		if (ret == -EBUSY) {
			dev_info(dev, "mxt_stop wakeup enable\n");
...
static int mxt_handle_pdata(struct mxt_data *data)
{
	...
	data->pdata->t15_num_keys = ARRAY_SIZE(mxt_t15_keys);
	data->pdata->t15_keymap = mxt_t15_keys;
	/* i2c mode, reset, irq gpio info */
	data->pdata->reset_gpio = of_get_named_gpio_flags(np, "atmel,reset-gpio",
				0, &data->pdata->resetflags);
	data->pdata->irq_gpio = of_get_named_gpio_flags(np, "atmel,irq-gpio",
				0, &data->pdata->irqflags);
	/* Set default parameters */
	data->pdata->irqflags = IRQF_TRIGGER_LOW;
	/* power ldo gpio info*/
	data->pdata->power_ldo_gpio = of_get_named_gpio_flags(np, "atmel,power_ldo-gpio",
				0, &data->pdata->power_ldo_gpio_flags);
	if (data->pdata->power_ldo_gpio < 0)
		printk("%s, power_ldo_gpio=%ld\n", __func__, data->pdata->power_ldo_gpio);
